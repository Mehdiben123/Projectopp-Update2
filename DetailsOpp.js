...
//console.log('API reponse :',res);
  ...
  <TouchableOpacity onPress={() => showDatepicker('remise')}>
    <Text style={{textAlign:'center'}}>{moment(values.dateRemise).format('DD-MM-YYYY ')}</Text>
  </TouchableOpacity>
  </View>
  {showRemise && (
    <DateTimePicker
      value={values.dateRemise}
      mode={modeRemise}
      is24Hour={true}
      display="default"
      onChange={(event, selectedDate) =>
        onChangeRemise(event, selectedDate, 'remise')
      }
    />
  )}
</View>
...
<View style={{right:165}}>
  ...
  <TouchableOpacity onPress={() => showDatepicker('closing')}>
    <Text style={{textAlign:'center'}}>{moment(values.dateClosing).format('MM-YYYY')}</Text>
  </TouchableOpacity>
  </View>
  {showClosing && (
    <DateTimePicker
      ...
      display="default"
      onChange={(event, selectedDate) =>
        onChangeClosing(event, selectedDate, 'closing')
      }
    />
  )}
</View>
